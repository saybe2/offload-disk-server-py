...
- **Поиск** по файлам и папкам с фильтрами по типу
- **Публичные ссылки** для расшаривания файлов:
  - Срок действия (бессрочно или ограниченный)
  - Лимит скачиваний
  - Защита паролем
...
Параметры: `q` — запрос, `category` — категория файлов (images, videos, audio, documents, archives, code), `type` — тип результатов (files, folders, all), `min_size` / `max_size` — фильтры по размеру.

### Публичные ссылки

| Метод | URL | Описание |
|-------|-----|----------|
| POST | `/api/files/<id>/share` | Создать ссылку на файл |
| GET | `/api/files/<id>/shares` | Список ссылок на файл |
| GET | `/api/shares` | Все мои ссылки |
| DELETE | `/api/shares/<id>` | Удалить ссылку |
| PATCH | `/api/shares/<id>/toggle` | Включить/отключить |
| GET | `/s/<token>` | Публичная страница скачивания |
| GET | `/s/<token>/download` | Скачивание по ссылке |
| GET | `/s/<token>/info` | Информация о ссылке (JSON) |
